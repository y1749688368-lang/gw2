...
namespace GUIStyle {
    ...
    bool GUIStyle::LoadAppFont(float fontSize) {
        ...
        if (!fontsDir.empty()) {
            // 尝试加载微软雅黑（支持中文，Windows 系统自带）
            std::string msyhPath = fontsDir + "\\msyh.ttc";
            ImFont* msyhFont = io.Fonts->AddFontFromFileTTF(
                msyhPath.c_str(),
                fontSize,
                NULL,
                io.Fonts->GetGlyphRangesChineseFull() // 加载完整中文字形
            );

            if (msyhFont) {
                io.FontDefault = msyhFont;
                success = true;
            }
            else {
                // 微软雅黑加载失败，尝试宋体
                std::string simsunPath = fontsDir + "\\simsun.ttc";
                ImFont* simsunFont = io.Fonts->AddFontFromFileTTF(
                    simsunPath.c_str(),
                    fontSize,
                    NULL,
                ...
                if (simsunFont) {
                    io.FontDefault = simsunFont;
                    success = true;
                }
                else {
                    MessageBoxA(NULL, "未找到支持中文的字体（微软雅黑/宋体），中文可能显示异常！", "字体警告", MB_OK | MB_ICONWARNING);
                }
            }
        }
        else {
            MessageBoxA(NULL, "无法获取系统字体目录，中文显示可能异常！", "字体错误", MB_OK | MB_ICONERROR);
        }
        return success;
    }
