...
#include <windows.h>  // ÓÃÓÚÎÄ¼þ¶Ô»°¿ò
#include <commctrl.h> // Í¨ÓÃ¿Ø¼þ
#include <commdlg.h>  // ÎÄ¼þ¶Ô»°¿òAPI
...
void Hack::readXYZ1() {
    ...
    if (m_xAddr != 0) m_memoryManager.Read<float>(m_xAddr, m_xValue1);
    if (m_yAddr != 0) m_memoryManager.Read<float>(m_yAddr, m_yValue1);
    if (m_zAddr != 0) m_memoryManager.Read<float>(m_zAddr, m_zValue1);
}

void Hack::writeXYZ1(float xValue, float yValue, float zValue)
{
    if (!m_memoryManager.IsAttached()) return;
...
void Hack::readXYZ2() {
    if (!m_memoryManager.IsAttached()) return;
    if (m_xAddr != 0) m_memoryManager.Read<float>(m_xAddr, m_xValue2);
    if (m_yAddr != 0) m_memoryManager.Read<float>(m_yAddr, m_yValue2);
    if (m_zAddr != 0) m_memoryManager.Read<float>(m_zAddr, m_zValue2);
}

void Hack::writeXYZ2(float xValue, float yValue, float zValue)
{
    if (!m_memoryManager.IsAttached()) return;
    if (m_xAddr != 0) m_memoryManager.Write<float>(m_xAddr, xValue);
    if (m_yAddr != 0) m_memoryManager.Write<float>(m_yAddr, yValue);
    ...
    if (!m_memoryManager.IsAttached()) return;
    if (m_xAddr != 0) m_memoryManager.Read<float>(m_xAddr, m_xValue3);
    if (m_yAddr != 0) m_memoryManager.Read<float>(m_yAddr, m_yValue3);
    if (m_zAddr != 0) m_memoryManager.Read<float>(m_zAddr, m_zValue3);
}

void Hack::writeXYZ3(float xValue, float yValue, float zValue)
{
    if (!m_memoryManager.IsAttached()) return;
    if (m_xAddr != 0) m_memoryManager.Write<float>(m_xAddr, xValue);
    if (m_yAddr != 0) m_memoryManager.Write<float>(m_yAddr, yValue);
...
void Hack::writeXYZmap(float xValue, float yValue, float zValue)
{
    if (!m_memoryManager.IsAttached()) return;
    if (m_xAddr != 0) m_memoryManager.Write<float>(m_xAddr, xValue);
    if (m_yAddr != 0) m_memoryManager.Write<float>(m_yAddr, yValue);
...
void Hack::readXYZmap() {
    if (!m_memoryManager.IsAttached()) return;
    if (m_xAddr != 0) m_memoryManager.Read<float>(m_xAddr, m_xValuemap);
    if (m_yAddr != 0) m_memoryManager.Read<float>(m_yAddr, m_yValuemap);
    if (m_zAddr != 0) m_memoryManager.Read<float>(m_zAddr, m_zValuemap);
}
...
void Hack::Maploadposition() {
    if (!m_memoryManager.IsAttached()) {
        reportStatus("ERROR: Cannot load position - process not attached.");
        return;
    }
    ...
    if (!GetOpenFileNameA(&ofn)) {
        reportStatus("INFO: User canceled file selection.");
        return;
    }

    std::ifstream file(filePath);
    if (!file.is_open()) {
        reportStatus("ERROR: Failed to open file: " + std::string(filePath));
        return;
    }
    ...
    try {
        ...
        reportStatus("ERROR: Invalid JSON file format.");
        file.close();
        return;
    }
    ...
    if (jsonData.contains("Coordinates") && jsonData["Coordinates"].is_array()) {
        for (const auto& item : jsonData["Coordinates"]) {
            std::string name = item.contains("Name") ? item["Name"].get<std::string>() : "Unknown";
            float x = item.contains("X") ? item["X"].get<float>() : 0.0f;
            float z = item.contains("Y") ? item["Y"].get<float>() : 0.0f; 
            float y = item.contains("Z") ? item["Z"].get<float>() : 0.0f;  
            m_jsonPoints.emplace_back(name, x, y, z);
        }
    }
    else {
        reportStatus("WARNING: No valid Coordinates array found in JSON file.");
    }

    m_showJsonViewer = true;
    reportStatus("INFO: Loaded " + std::to_string(m_jsonPoints.size()) + " points from JSON.");
}

void Hack::Mapcreateposition() {
    readXYZmap();

    const std::string filePath = R"(C:\Users\Crazy\Desktop\positions.json)";
    ...
    try {
        ...
        if (readFile.is_open()) {
            try {
                ...
                // 文件格式错误时，初始化根对象并创建Coordinates数组
                jsonData = json::object();
                jsonData["Coordinates"] = json::array();
                reportStatus("WARN: JSON file format error, reset to new structure");
            }
            readFile.close();
        }
        else {
        ...
        std::string defaultName = "Point_" + std::to_string(coordinates.size() + 1);
        ...
        if (writeFile.is_open()) {
            writeFile << std::setw(4) << jsonData; 
            writeFile.close();
            reportStatus("INFO: Position saved as '" + defaultName + "'");
        }
        else {
            reportStatus("ERROR: Failed to open file for writing (Path: " + filePath + ")");
        }
    }
    catch (const std::exception& e) {
        reportStatus("ERROR: Save failed - " + std::string(e.what()));
    }
}
void Hack::savePosition1() {
    readXYZ1();
    m_xSave1 = m_xValue1;
    m_ySave1 = m_yValue1;
    m_zSave1 = m_zValue1;
...
void Hack::loadPosition1() {
    ...
    if (m_xSave1 != 0.0f || m_ySave1 != 0.0f || m_zSave1 != 0.0f) {
        writeXYZ1(m_xSave1, m_ySave1, m_zSave1);
...
void Hack::loadPosition2() {
    if (m_xAddr == 0 || m_yAddr == 0 || m_zAddr == 0) {
        reportStatus("ERROR: Cannot load position, coordinate addresses not resolved.");
        return;
    }
    // Basic check if position has ever been saved to non-zero coords
    if (m_xSave2 != 0.0f || m_ySave2 != 0.0f || m_zSave2 != 0.0f) {
        writeXYZ2(m_xSave2, m_ySave2, m_zSave2);
        reportStatus("INFO: Position loaded.");
    }
    else {
        reportStatus("WARN: No position saved to load.");
...
void Hack::loadPosition3() {
    if (m_xAddr == 0 || m_yAddr == 0 || m_zAddr == 0) {
        reportStatus("ERROR: Cannot load position, coordinate addresses not resolved.");
        return;
    }
    // Basic check if position has ever been saved to non-zero coords
    if (m_xSave3 != 0.0f || m_ySave3 != 0.0f || m_zSave3 != 0.0f) {
        writeXYZ3(m_xSave3, m_ySave3, m_zSave3);
        reportStatus("INFO: Position loaded.");
    }
    else {
        reportStatus("WARN: No position saved to load.");
