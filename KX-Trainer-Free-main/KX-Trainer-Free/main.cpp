...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    while (!done) {
        ...
        if (isLoading) { // Only true if status check passed and background task is running
            ...
            //mapload
            if (g_hack && g_hack->m_showJsonViewer) {
                ImGui::SetNextWindowPos(ImGui::GetMainViewport()->GetCenter(), ImGuiCond_FirstUseEver, ImVec2(0.5f, 0.5f));
                ImGui::SetNextWindowSize(ImVec2(800, 400), ImGuiCond_FirstUseEver);
                ImGui::Begin("JSON Points Viewer", &g_hack->m_showJsonViewer, ImGuiWindowFlags_NoCollapse);
                ImGui::TextColored(ImVec4(0.3f, 0.9f, 0.3f, 1.0f), "Loaded Points: %zu", g_hack->m_jsonPoints.size());

                ImGui::Separator();


                ImGui::BeginChild("PointsScrollArea", ImVec2(0, -50), true);
                for (size_t i = 0; i < g_hack->m_jsonPoints.size(); ++i) {
                    const auto& [name, x, y, z] = g_hack->m_jsonPoints[i];
                    ...
                    ImGui::SameLine(550);
                    std::string btnLabel = "Position Load##" + std::to_string(i);
                    if (ImGui::Button(btnLabel.c_str(), ImVec2(120, 25))) {
                        g_hack->loadPoint(x, y, z); // 点击时调用加载函数
                    }
                    if (i != g_hack->m_jsonPoints.size() - 1) ImGui::Separator();
                }
                ImGui::EndChild();


                if (ImGui::Button("Close", ImVec2(120, 30))) {
                    g_hack->m_showJsonViewer = false;
                }
                ImGui::End();
            }
